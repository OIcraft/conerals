	...
	void getMap() {
		...
		for (int i = 1; i <= mapRows; ++i) {
			for (int j = 1; j <= mapCols; ++j) {
				...
				if(PhirainEX_map[i][j].type == 'K' && PhirainEX_map[i][j].belongTo != PhirainEX_id) {
					if(new_map.type == 'C'||new_map.type == 'K') {
						PhirainEX_map[i][j]=new_map;
					} else {
						PhirainEX_map[i][j].army=10;
					}
				} else {
					PhirainEX_map[i][j]=new_map;
				}
		...
		for (int i = 1; i <= mapRows; ++i) {
			for (int j = 1; j <= mapCols; ++j) {
				...
				if(PhirainEX_map[i][j].belongTo == 0||isinvis(i,j)) {
					PhirainEX_val=1;
					if(PhirainEX_map[i][j].type == 'C') PhirainEX_val=2;
				} else {
					PhirainEX_val=2;
					if(PhirainEX_map[i][j].type == 'C') PhirainEX_val=3;
					if(PhirainEX_map[i][j].type == 'K') PhirainEX_val=4;
				}
				if((PhirainEX_val>max_val)||
				    (PhirainEX_val==max_val&&PhirainEX_dis[i][j]<min_dis)||
					(PhirainEX_val==max_val&&PhirainEX_dis[i][j]==min_dis&&PhirainEX_num[i][j]>max_army)) {
					min_dis=PhirainEX_dis[i][j];
					max_army=PhirainEX_num[i][j];
